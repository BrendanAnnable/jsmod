 ...
 * @param {!Object} parseOptions Parsing options to pass to recast.
 * @returns {!P<!JSMod>} A promise to a JSMod project instance.
 */
function load(filePatterns, parseOptions) {
  return Promise.all(filePatterns.map(filePattern => glob(filePattern)))
    .then(filePathsList => [].concat(...filePathsList))
    .then(filePaths => Promise.all(parseFiles(filePaths, parseOptions)))
    .then(files => {
      const original = jsmod(build.project(files));
      return {
        project: original,
        save: (project, printOptions) => save(project, original, printOptions)
      };
    });
}
 ...
 * @param {?Object=} printOptions Printing options to pass to recast.
 * @returns {!P} A promise that is resolved when all files are written to disk.
 */
function save(project, original, printOptions) {
    ...
    // Filter by those which have changed.
      ...
      .map(file => {
        return ensureDirectoryExists(file.name)
          ...
          .then(() => {
            const source = toSource(file, printOptions);
 ...
 * @param {?Object=} printOptions Parsing options to pass to recast.
 * @returns {string} The source code of the file.
 */
function toSource(file, printOptions) {
  return recast.print(file, printOptions).code;
}
